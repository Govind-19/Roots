...
export default function BottomNav({ activeTab, setActiveTab, isWarning }) {
    const navItems = [
    ...
    const activeColor = isWarning ? 'text-[var(--theme-accent-800)]' : 'text-nature-800';
            ...
            <div className="flex justify-around items-center h-20 px-4 max-w-md mx-auto">
                {navItems.map((item) => (
                    <button
                        ...
                        className={cn(
                            "flex flex-col items-center justify-center w-full h-full space-y-1 transition-all duration-300 theme-transition",
                            activeTab === item.id ? `${activeColor} -translate-y-1` : "text-earth-600 hover:text-nature-700"
                        )}
                    >
